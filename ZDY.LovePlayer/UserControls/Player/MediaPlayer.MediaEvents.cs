...
namespace ZDY.LovePlayer.UserControls
{
    public partial class MediaPlayer
    {
        ...
        private void OnMediaInitializing(object sender, MediaInitializingRoutedEventArgs e)
        {
            ...
            MediaPlayerElement.RendererOptions.AudioDisableSync = e.Url.EndsWith(".wmv");
        ...
        private void OnMediaOpening(object sender, MediaOpeningRoutedEventArgs e)
        {
            ...
            if (videoStream != null)
            {
                ...
                {
                    MediaPlayerElement.ClosedCaptionsChannel = videoStream.HasClosedCaptions ?
        ...
        private async void OnMediaOpened(object sender, RoutedEventArgs e)
        {
            ...
            {
                if (string.IsNullOrEmpty(ViewModel.Media.Title))
                {
                    var url = ((Unosquare.FFME.MediaElement)e.OriginalSource).MediaInfo.InputUrl;
                    var index = url.LastIndexOf("\\");
                    if (index != -1)
                    {
                        ViewModel.Media.Title = url.Substring(index + 1);
                    }
                    else
                    {
                        ViewModel.Media.Title = "未命名视频";
                    }
                }
                await ViewModel.RecordSeenMovieAsync();
            }
        }
        ...
        //    await MediaPlayerElement?.ChangeMedia();
        ...
        {
            Debug.WriteLine($"{nameof(MediaPlayerElement.Position)} = {e.Position}");
        }
